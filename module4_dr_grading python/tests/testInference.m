...
moduleRoot = fileparts(testDir);
addpath(moduleRoot);
addpath(fullfile(moduleRoot, 'src'));
addpath(fullfile(moduleRoot, 'src', 'utils'));

% Define model file path early to avoid undefined variable in catch block
modelFile = fullfile(moduleRoot, 'trained_models', 'mock_model_test.mat');
...
try
    ...
    if ~exist(fullfile(moduleRoot, 'trained_models'), 'dir')
        mkdir(fullfile(moduleRoot, 'trained_models'));
    end
...
% Clean up paths
rmpath(moduleRoot);
rmpath(fullfile(moduleRoot, 'src'));
rmpath(fullfile(moduleRoot, 'src', 'utils'));
