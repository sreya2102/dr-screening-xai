...
dummyFolder = fullfile(moduleRoot, 'data', 'dummy');
needGeneration = false;
if ~exist(dummyFolder, 'dir')
    needGeneration = true;
else
    % Check if all 5 class directories exist and contain at least one image
    for c = 0:4
        classFolder = fullfile(dummyFolder, num2str(c));
        if ~exist(classFolder, 'dir')
            needGeneration = true;
            break;
        else
            files = dir(fullfile(classFolder, '*.png'));
            if isempty(files)
                needGeneration = true;
                break;
            end
